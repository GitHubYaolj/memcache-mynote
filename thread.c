...
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    event_set(&me->notify_event, me->notify_receive_fd,//worker线程 监听管道读事件
              EV_READ | EV_PERSIST, thread_libevent_process, me);
    event_base_set(me->base, &me->notify_event);//将event_base和event关联
...
static void *worker_libevent(void *arg) {
    ...
    event_base_loop(me->base, 0);//worker线程进入事件循环
    return NULL;
}
 ...
 */
static void thread_libevent_process(int fd, short which, void *arg) {//worker线程管道可读事件，有主线程新发来的连接socket
    LIBEVENT_THREAD *me = arg;
    CQ_ITEM *item;
    char buf[1];
    ...
    case 'c':
        ...
        if (NULL != item) {
            conn *c = conn_new(item->sfd, item->init_state, item->event_flags,
                               ...
                               me->base);//更换了event_base，但回调函数是event_handler，状态机
            if (c == NULL) {
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
    ...
    int tid = (last_thread + 1) % settings.num_threads;//轮询

    LIBEVENT_THREAD *thread = threads + tid;//选定一个worker线程
    ...
    buf[0] = 'c';
    if (write(thread->notify_send_fd, buf, 1) != 1) {//会触发worker线程的管道可读事件回调函数thread_libevent_process
...
void memcached_thread_init(int nthreads) {
    ...
    for (i = 0; i < nthreads; i++) {
        int fds[2];//为每个worker线程分配一个管道，用于通知worker线程
        if (pipe(fds)) {
        ...
        setup_thread(&threads[i]);//为每个worker线程分配一个event_base，监听notify_receive_fd读事件，同时分配一个conn_queue队列
    ...
    for (i = 0; i < nthreads; i++) {
        create_worker(worker_libevent, &threads[i]);//工作线程执行函数,进入事件循环
    }
