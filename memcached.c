...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    event_set(&c->event, sfd, event_flags, event_handler, (void *)c);//主线程监听socket可读回调函数，worker线程中的socket可读可写事件
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        switch(c->state) {
        case conn_listening://主线程监听socket 可读回调函数
            addrlen = sizeof(addr);
#ifdef HAVE_ACCEPT4
            if (use_accept4) {
            ...
            if (settings.maxconns_fast &&
                ...
                STATS_UNLOCK();
            } else {//选定一个worker线程，new一个CQ_ITEM,把这个CQ_ITEM扔给这个线程
...
void event_handler(const int fd, const short which, void *arg) {
    ...
    drive_machine(c);//有新连接时，c->state == conn_listening
...
static int server_socket(const char *interface,
    ...
    for (next= ai; next; next= next->ai_next) {
        ...
        if ((sfd = new_socket(next)) == -1) {//创建一个socket，设置为非阻塞
        ...
        if (IS_UDP(transport)) {
            ...
            if (!(listen_conn_add = conn_new(sfd, conn_listening,
                                             ...
                                             transport, main_base))) {//监听socket 事件 添加进主进程base
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        for (char *p = strtok_r(list, ";,", &b);
            ...
            ret |= server_socket(p, the_port, transport, portnumber_file);//创建监听socket
        }
        free(list);
        return ret;
    }
...
int main (int argc, char **argv) {
    ...
    memcached_thread_init(settings.num_threads);//创建num_threads个worker线程,为每个worker线程创建一个CQ队列,worker线程进入事件循环
    ...
    clock_handler(0, 0, 0);//主线程定时更新current_time(1s)
    ...
    if (settings.socketpath == NULL) {
        ...
        errno = 0;
        if (settings.udpport && server_sockets(settings.udpport, udp_transport,//listening socket,创建一个event，监听该socket可读事件(可能多个ip,对应多个socket)
                                              portnumber_file)) {
